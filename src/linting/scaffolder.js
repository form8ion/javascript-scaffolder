...
export default async function ({projectRoot, tests, configs, vcs, transpileLint, buildDirectory}) {
  const [eslintResult, remarkResult, banSensitiveFilesResult] = await Promise.all([
    configs.eslint && false !== transpileLint
      ? scaffoldEslint({projectRoot, unitTested: tests.unit, config: configs.eslint, buildDirectory})
      : null,
